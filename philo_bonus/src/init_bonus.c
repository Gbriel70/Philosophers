...
short set_unlinked_semaphores(t_semaphores *semaphores, int nbr_philos)
{
    ...
    if 
    (
    semaphores->forks == SEM_FAILED || \
    semaphores->table == SEM_FAILED || \
    semaphores->print == SEM_FAILED || \
    semaphores->death == SEM_FAILED || \
    semaphores->full == SEM_FAILED || \
    semaphores->stop == SEM_FAILED || \
